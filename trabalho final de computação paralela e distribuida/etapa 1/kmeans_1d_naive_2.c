...
static double assignment_step_1d(const double *X, const double *C, int *assign, int N, int K){
    ...
    // AQUI ESTÁ A CHAVE: 'reduction(+:sse)' garante que a soma ocorra
    // de forma eficiente e sem locks entre as iterações do loop.
    ...
    for(int i=0;i<N;i++){
        ...
        assign[i] = best;
        sse += bestd; // Soma local (privada da thread)
    }
    return sse;
}
...
static void update_step_1d(const double *X, double *C, const int *assign, int N, int K){
    ...
    // MODIFICAÇÃO 1: Paraleliza o laço a seguir
    #pragma omp parallel for
    for(int i=0; i<N; i++){
        int a = assign[i];
        
        // MODIFICAÇÃO 2: Protege o trecho que altera memória compartilhada
        #pragma omp critical
        {
            cnt[a] += 1;
            sum[a] += X[i];
    ...
    // Este laço continua sequencial, pois ele só roda depois que o paralelo terminou
    for(int c=0; c<K; c++){
